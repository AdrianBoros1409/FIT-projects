...
void phong_vertexShader(GPUVertexShaderOutput *const      output,
	...
	Mat4 const *const view = shader_interpretUniformAsMat4(UniformHandle, viewMatrixLocation);

	UniformLocation const projectionMatrixLocation = getUniformLocation(gpu, "projectionMatrix");
	Mat4 const *const proj = shader_interpretUniformAsMat4(UniformHandle, projectionMatrixLocation);
	...
	copy_Vec3(positionOut, position);
	copy_Vec3(normalOut, normal);
}
