...
void const *gpu_computeVertexAttributeDataPointer(
	...
	(void)head;
	(void)gl_VertexID;
	return (head->offset + (char *)head->buffer + (head->stride * gl_VertexID));
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
	...
	VertexIndex VertexID = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);
	GPUVertexPullerHead const *head;
	for (size_t i = 0; i < MAX_ATTRIBUTES; i++) {
		head = &puller->heads[i];
		if (head->enabled)
			output->attributes[i] = gpu_computeVertexAttributeDataPointer(head, VertexID);
		else
...
void gpu_runPrimitiveAssembly(
	...
	GPUVertexShaderOutput *shader_output = primitive->vertices;
	for (size_t i = baseVertexShaderInvocation; i < nofPrimitiveVertices + baseVertexShaderInvocation; i++) {
		GPUVertexPullerOutput puller_output;
		gpu_runVertexPuller(&puller_output, puller, i);
		GPUVertexShaderInput shader_input = { .attributes = &puller_output };
		shader_input.gl_VertexID = gpu_computeGLVertexID(puller->indices, i);
		vertexShader(shader_output++, &shader_input, gpu);
	}
...
void gpu_computeScreenSpaceBarycentrics(
	...
	/*
	Funkciu som vypracoval podla stranky:
	https://stackoverflow.com/questions/25385361/point-within-a-triangle-barycentric-co-ordinates
	*/
	float det = ((vertices[1].data[1] - vertices[2].data[1])*(vertices[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0])*(vertices[0].data[1] - vertices[2].data[1]));
	float factor_alpha = ((vertices[1].data[1] - vertices[2].data[1])*(pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0])*(pixelCenter[0].data[1] - vertices[2].data[1]));
	float factor_beta = ((vertices[2].data[1] - vertices[0].data[1])*(pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0])*(pixelCenter[0].data[1] - vertices[2].data[1]));
	coords->data[0] = factor_alpha / det;
	coords->data[1] = factor_beta / det;
