...
void pinedaPolygon(const Point *points, const int size, const RGBA &color1, const RGBA &color2)
{
	...
	for (int i = 0; i < size - 1; i++) {
		edgeParam[i].deltaX = points[i + 1].x - points[i].x;
		edgeParam[i].deltaY = points[i + 1].y - points[i].y;
	}

	edgeParam[size - 1].deltaX = points[0].x - points[size - 1].x;
	edgeParam[size - 1].deltaY = points[0].y - points[size - 1].y;
