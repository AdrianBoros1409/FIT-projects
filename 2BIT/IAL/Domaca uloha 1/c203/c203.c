...
/*	Vypracoval: Adrián Boros [xboros3], 2018/2019						      */
/* ************************************************************************** */
/*
...
*/
	if (q == NULL)	//ak sa q==NULL vola sa funkcia queueError(QERR_INIT)
		queueError(QERR_INIT);
	else
	{
		for (int i = 0; i < QUEUE_SIZE; i++)	//nastavenie hodnot v poli q->arr na '*'
		{
			q->arr[i] = '*';
		}
		q->b_index = 0;	//nastavenie indexov na 0
		q->f_index = 0;
	}
}
...
*/
	return ((index + 1) % QUEUE_SIZE);	//vzorec je (qBeg + 1) % QMAX
}
...
*/
	return ((q->b_index == q->f_index) ? 1 : FALSE);	//ak je prvy prvok zaroven aj prvym prazdnym prvkom vrati sa 1, inak sa vracia 0
}
...
*/
	return ((nextIndex(q->b_index) == q->f_index) ? 1 : FALSE);	//ak je nasledujuci index b_index rovnaky ako f_index tak fronta je plna
}
...
*/
	if (queueEmpty(q) != 0)	//ak je fronta prazdna, funkcia queueEmpty vracia nenulovu hodnotu
		queueError(QERR_FRONT);
	else
		*c = q->arr[q->f_index];	//ak fronta nie je prazdna, vracia sa znak zo zaciatku fronty
}
...
*/
	if (queueEmpty(q) != 0)	//ak je fronta prazdna, vola sa funkcia queueError
		queueError(QERR_REMOVE);
	else
		q->f_index = nextIndex(q->f_index);	//index prveho prvku sa posunie na index nasledujuceho prvku
}
...
*/
	...
	{
		queueFront(q, c);	//funkcia vracia prostrednictvom c znak zo zaciatku fronty
		queueRemove(q);	//funkcia odstrani znak zo zaciatku fronty
	}
}
...
*/
	if (queueFull(q) != 0)	////ak je fronta plna, vola sa funkcia queueError
		queueError(QERR_UP);
	else
	{
		q->arr[q->b_index] = c;	//vkladanie znaku c na prvu volnu poziciu
		q->b_index = nextIndex(q->b_index);	//posunute b_index na dalsiu volnu poziciu
	}
}
